...
# تلگرام
# ---------------------------------------------------------------------------
TELEGRAM_BOT_TOKEN = os.getenv("8890210647:AAE_64GUSExXMA7klRvAzQVqtWvlU0L5EXw", "")
TELEGRAM_CHAT_ID = os.getenv("292739287", "")
...
config.py
---------
تمام تنظیمات پروژه از طریق متغیرهای محیطی (Environment Variables) خوانده می‌شود.
"""
...
def validate_telegram_config() -> None:
    ...
    missing = []
    if not TELEGRAM_BOT_TOKEN:
        missing.append("8890210647:AAE_64GUSExXMA7klRvAzQVqtWvlU0L5EXw")
    if not TELEGRAM_CHAT_ID:
        missing.append("292739287")
    if missing:
        raise RuntimeError(
            "متغیرهای محیطی زیر تنظیم نشده‌اند: " + ", ".join(missing)
        )
