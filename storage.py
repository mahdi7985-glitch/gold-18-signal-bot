...
# ============================================
# توابع جدید برای تغییرات قیمت و روند
# ============================================
...
def get_recent_prices(days: int = 5) -> List[float]:
    """
    دریافت لیستی از قیمت‌های اخیر به تومان
    
    Args:
        days: تعداد روزهای اخیر
    
    Returns:
        list: لیست قیمت‌ها به تومان (از قدیم به جدید)
    """
    history = load_history()
    if history.empty:
        return []
...
def get_weekly_trend() -> Dict[str, Any]:
    """
    محاسبه روند هفتگی قیمت طلا
    
    Returns:
        dict: {
            'high': float,      # بالاترین قیمت هفته (تومان)
            'low': float,       # پایین‌ترین قیمت هفته (تومان)
            'start': float,     # قیمت اول هفته (تومان)
            'end': float,       # قیمت آخر هفته (تومان)
            'change': float,    # تغییرات درصدی
            'trend': str        # 'صعودی', 'نزولی', 'خنثی'
        }
    """
    history = load_history()
    if len(history) < 5:
    ...
    high = max(prices_toman)
    low = min(prices_toman)
    start = prices_toman[0]
    end = prices_toman[-1]
...
if __name__ == "__main__":
    ...
    # تست قیمت‌های اخیر
    recent = get_recent_prices(5)
    print(f"📅 قیمت‌های اخیر (تومان): {[f'{p:,.0f}' for p in recent]}")
